...
class _ToDoScreenState extends State<ToDoScreen> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          SingleChildScrollView(
            child: Container(
              margin: EdgeInsets.symmetric(
                  horizontal: 12, vertical: SizeConfig.orientation == Orientation.landscape ? 0 : 15),
              child: Row(
                    ...
                    children: [
                      Text(
                        ...
                        style: const TextStyle(
                          //AppLocalizations.of(context)!.today_word,
                          fontSize: 26,
                          fontWeight: FontWeight.bold,
                        ),
                      ),
                      Text(
                        intl.DateFormat.yMMMMd(/*AppLocalizations.of(context)!.language*/).format(DateTime.now()),
                        style: TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                          color:
                              ThemeData.fallback().brightness == Brightness.dark ? Colors.grey.shade400 : Colors.grey,
                        ),
                      ),
                    ],
                  ),
          ...
          todoProvider.taskList.isEmpty
              ? Stack(
                  children: [
                    AnimatedPositioned(
                      duration: const Duration(milliseconds: 1000),
                      child: RefreshIndicator(
                        onRefresh: () => todoProviderForMethods.getTasks(),
                          ...
                          child: Wrap(
                            ...
                            children: [
                              SizeConfig.orientation == Orientation.landscape
                                  ...
                                  : SizedBox(height: SizeConfig.getProportionateScreenHeight(220.0)),
                              Icon(Icons.add),
                              Padding(
                  ...
                  child: RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (ctx, index) {
                        Task task = todoProvider.taskList[index];

                        if ((task.repeat == 'Daily' || task.repeat == 'التكرار يوميا') ||
                            task.date == intl.DateFormat.yMMMMEEEEd().format(_selectedDate) ||
                            ((task.repeat == 'Weekly' || task.repeat == 'التكرار اسبوعيا') &&
                                _selectedDate.difference(intl.DateFormat.yMMMMEEEEd().parse(task.date!)).inDays % 7 ==
                                    0) ||
                            ((task.repeat == 'Monthly' || task.repeat == 'التكرار شهريا') &&
                                intl.DateFormat.yMMMMEEEEd().parse(task.date!).day == _selectedDate.day)) {
                            ...
                            child: SlideAnimation(
                              ...
                              child: FadeInAnimation(
                                child: GestureDetector(
                                  onTap: () => _showBottomSheet(context, task, todoProvider),
                                  child: TaskTile(task),
                                ),
                              ),
                            ),
  ...
  _showBottomSheet(BuildContext ctx, Task task, todoProviderForMethods) {
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              _buildBottomSheet(
                  onTap: () {
                    todoProviderForMethods.markAsCompleted(task: task);
                    Navigator.of(ctx).pop();
                  },
                  color: Colors.green,
                  label: task.isCompleted == 1
                      ? "Mark as To Do"
                      : "Mark as complete"), //AppLocalizations.of(context)!.assign_to_do:AppLocalizations.of(context)!.complete),
              _buildBottomSheet(
                  onTap: () {
                    todoProviderForMethods.deleteTasks(task);
                    Navigator.of(ctx).pop();
                  },
                  color: Colors.red,
  ...
  _buildBottomSheet({required String label, required Function() onTap, required Color color, bool isClose = false}) {
